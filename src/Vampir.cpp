	...
	if (isBat) {
		...
		*/
		...
		if (collisionUp && collisionDown) {
			...
			int collisionUp = map->collisionMoveUp(nextPos, realColliderOffset, glm::ivec2(realColliderSize.x, 1));
			int collisionDown = map->collisionMoveDown(glm::ivec2(nextPos.x, nextPos.y + realColliderSize.y - 1), realColliderOffset, glm::ivec2(realColliderSize.x, 1));
